...
FileServer::onStartServer() {
    ...
    if(port == 0) {
        logMessage(logFile,
                   sFunctionName,
                   serverName +
                   QString(tr(" Error! Server port not set.")));
    ...
    for(int i=0; i<nConnections; i++) {
        if(connections.at(i)->peerAddress() == pClient->peerAddress()) {
            logMessage(logFile,
                       sFunctionName,
                       serverName +
                       QString(tr(" %1 Duplicate requests from %2"))
                       .arg(sServerName)
            ...
            if(connections.at(i)->isValid() && pClient->isValid()) {
                logMessage(logFile,
                           sFunctionName,
                           serverName +
                           QString(tr(" Both sockets are valid! Removing the old connection")));
                connections.at(i)->close(QWebSocketProtocol::CloseCodeNormal,
                                         QString(tr("Duplicated request")));
                ...
                if(pClient->isValid()) {
                    logMessage(logFile,
                               sFunctionName,
                               serverName +
                               QString(tr(" Only present socket is valid. Removing the old one")));
                    connections.at(i)->close(QWebSocketProtocol::CloseCodeNormal,
                                             QString(tr("Duplicated request")));
                    delete connections.at(i);
                }
                    ...
                    logMessage(logFile,
                               sFunctionName,
                               serverName +
                               QString(tr(" Present socket is not valid.")));
...
FileServer::onClientSocketError(QAbstractSocket::SocketError error) {
    ...
    if(!disconnect(pClient, 0, 0, 0)) {
        logMessage(logFile,
                   sFunctionName,
                   serverName +
                   QString(tr(" Unable to disconnect signals from WebSocket")));
    }
    pClient->abort();
    if(!connections.removeOne(pClient)) {
        logMessage(logFile,
                   sFunctionName,
                   serverName +
                   QString(tr(" Unable to remove %1 from list !"))
                   .arg(pClient->peerAddress().toString()));
    }
    delete pClient;
}
...
FileServer::onProcessTextMessage(QString sMessage) {
    ...
    if(sToken != sNoData) {
        ...
        if(argumentList.count() < 3) {
            logMessage(logFile,
                       sFunctionName,
                       QString(tr("Bad formatted requests: %1"))
                       .arg(sToken));
            return;
        }
        ...
        logMessage(logFile,
                   sFunctionName,
                   QString(tr("%1 asked for: %2 %3 bytes, starting form %4 "))
                   .arg(pClient->peerAddress().toString())
                   .arg(sFileName)
                   .arg(length)
                   .arg(startPos));
        ...
        if(file.exists()) {
            ...
            if(filesize <= startPos) {
                logMessage(logFile,
                           sFunctionName,
                           QString(tr("File size %1 is less than requested start position: %2"))
                           .arg(filesize)
                           .arg(startPos));
                return;
            }
            ...
            if(file.open(QIODevice::ReadOnly)) {
                if(file.seek(startPos)) {
                    ...
                    if(pClient->isValid()) {
                        ...
                        if(bytesSent != ba.count()) {
                            logMessage(logFile,
                                       sFunctionName,
                                       QString(tr("Unable to send the file %1"))
                                       .arg(sFileName));
                        }
#ifdef LOG_VERBOSE
                        else {
                        ...
                        logMessage(logFile,
                                   sFunctionName,
                                   QString(tr("Client disconnected while sending %1"))
                                   .arg(sFileName));
                        return;
                    }
                    return;
                    ...
                    logMessage(logFile,
                               sFunctionName,
                               QString(tr("Error seeking %1 to %2"))
                               .arg(sFileName)
                               .arg(startPos));
                    return;
                }
                ...
                logMessage(logFile,
                           sFunctionName,
                           QString(tr("Unable to open the file %1"))
                           .arg(sFileName));
                return;
            }
        }
...
FileServer::senderThreadFinished() {
    ...
    logMessage(logFile,
               sFunctionName,
               serverName +
               QString(tr(" Sender Thread terminated")));
...
FileServer::onFileTransferDone(bool bSuccess) {
    ...
    logMessage(logFile,
               sFunctionName,
               serverName +
               QString(tr(" File Transfer terminated with code %1"))
               .arg(bSuccess));
}
    ...
    if (pClient->isValid()) {
        ...
        if(written != sMessage.length()) {
            logMessage(logFile,
                       serverName +
                       sFunctionName,
                       QString(tr(" Error writing %1")).arg(sMessage));
        }
#ifdef LOG_VERBOSE
        else {
            logMessage(logFile,
        ...
        logMessage(logFile,
                   sFunctionName,
                   serverName +
                   QString(tr(" Client socket is invalid !")));
    }
    return 0;
}
...
FileServer::onProcessBinaryMessage(QByteArray message) {
    ...
    logMessage(logFile,
               sFunctionName,
               QString(tr("Unexpected binary message received !")));
}
...
FileServer::onClientDisconnected() {
    ...
    logMessage(logFile,
               sFunctionName,
               serverName +
               QString(tr(" %1 disconnected because %2. Close code: %3"))
    ...
    for(int i=0; i<senderThreads.count(); i++) {
        ...
        if(senderThreads.at(i)->wait(3000)) {
            logMessage(logFile,
                       sFunctionName,
                       serverName +
                       QString(tr(" File Server Thread %1 regularly closed"))
                       .arg(i));
        }
        else {
            logMessage(logFile,
                       sFunctionName,
                       serverName +
                       QString(tr(" File Server Thread %1 forced to close"))
                       .arg(i));
